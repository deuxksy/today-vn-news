...
COMMON_INSTRUCTIONS = (
    "\n\n**출력 규칙 (최우선 지침):**\n"
    "- **중요: 만약 대상 기준일(오늘)에 해당하는 새로운 기사가 없다면, 어떠한 설명이나 텍스트도 출력하지 말고 반드시 '공백(Empty String)'만 반환하세요.**\n"
    "- **메타 정보 금지: '검색을 시작하겠습니다', '파일을 읽겠습니다'와 같은 내부 진행 과정이나 로봇의 대화, 인삿말을 절대 포함하지 마세요.** 출력 결과물은 TTS를 위해 즉시 사용되므로, **오직 뉴스 본문(헤더, 요약, 링크)만** 출력해야 합니다.\n"
    "- **일간지 이름은 반드시 ## (Level 2 Header)로 시작하세요.**\n"
    "- **각 기사 제목은 반드시 ### (Level 3 Header)로 시작하세요.**\n"
