...
def translate_articles(
    ...
    # Gemma 프롬프트 구성
    prompt = f"""다음 베트남어 뉴스 기사들을 한국어로 번역하고, 각각 3줄 요약을 작성해줘.
...
**중요**:
1. 제목과 내용을 자연스러운 한국어 문어체로 번역하세요.
...
def save_translated_yaml(translated_data: Dict, date_str: str, output_path: str) -> bool:
    """
    번역된 데이터를 YAML로 저장
    ...
    yaml_data = {
        ...
        "sections": translated_data["sections"] if isinstance(translated_data, dict) else translated_data
    }
...
def translate_and_save(scraped_data: Dict, date_str: str, output_path: str) -> bool:
    ...
    for source_name in source_order:
        ...
        if source_name == "안전 및 기상 관제":
            ...
            if section["items"]:
                ...
                print(f"  [OK] {source_name} 데이터 저장 완료: {len(section['items'])}개")
        ...
        # 일반 뉴스 소스 (스크래핑 + 번역)
            ...
            # 번역
            translated_items = translate_articles(articles, source_name, date_str, len(articles))
    ...
    # 번역된 YAML 저장
    return save_translated_yaml({"sections": translated_sections}, date_str, output_path)
