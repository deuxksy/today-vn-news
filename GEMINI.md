...
- **연속성 유지**: 세션 시작 시 **`docs/walkthroughs/`** 폴더 내 최신 문서를 참조하여 중단된 지점부터 문맥을 복구한다.
...
- **권한 요청**: `ContextFile.md` 및 `GEMINI.md`를 제외한 모든 파일 및 작업(삭제, 수정 등)은 사용자의 별도 승인 없이 자율적으로 수행한다. 단, `ContextFile.md` 또는 `GEMINI.md`를 수정하거나 삭제할 때만 반드시 사용자의 명시적 승인을 요청한다.
