...
- **연속성 유지**: 세션 시작 시 **`docs/walkthroughs/`** 폴더 내 최신 문서 및 **`git log`**를 참조하여 중단된 지점부터 문맥을 복구한다.
...
- **Data Flow**:
  - **수집**: Gemini API를 통해 뉴스 기사 데이터를 수집한다.
  - **TTS**: 수집된 뉴스 기사 데이터를 파싱하여 TTS 생성한다.
  - **영상**: TTS를 영상으로 변환한다.
  - **Youtube**: 영상을 업로드한다.
...
- **권한 요청**:
  - `ContextFile.md`,`GEMINI.md`, `docs/ROADMAP.md`는 절대 삭제 불가능.
  - `ContextFile.md`,`GEMINI.md`, `docs/ROADMAP.md`는 수정 할때만 사용자의 명시적 승인을 요청한다.
  - `ContextFile.md`,`GEMINI.md`, `docs/ROADMAP.md`를 제외한 모든 파일 및 작업(삭제, 수정 등)은 사용자의 별도 승인 없이 자율적으로 수행한다.
...
- **Release Policy**:
  ...
  - **내부 업데이트**: 릴리즈 사이의 세부 작업은 `0.0.x` 패치 버전으로 관리하며, docs/walkthroughs 에 기록하고 커밋한다. 태그/릴리즈는 생성하지 않는다.
